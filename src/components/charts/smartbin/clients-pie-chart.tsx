...
export function SmartbinClientsPieChart({ stats }: { stats: Record<string, number> }) {
  const id = "smartbin-clients-pie";
  ...
  if (chartData.length === 0) {
    return (
      <Card data-slot="card" className="@container/card @xl/main:col-span-2 gap-3">
        <CardHeader>
          <CardTitle>Smartbin Clients Distribution</CardTitle>
          <CardDescription>No data available for drop clients.</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-1 justify-center items-center min-h-[250px]">
          {" "}
          {/* Added min-h for consistency */}
          <p className="text-muted-foreground">No client data to display.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card data-slot="card" data-chart={id} className="@container/card gap-3">
      ...
      <CardHeader className="flex flex-row items-start space-y-0 pb-0">
        <div className="grid gap-1">
          <CardTitle>Smartbin Clients Distribution</CardTitle>
          <CardDescription>Shows the distribution of smartbin clients.</CardDescription>
        </div>
