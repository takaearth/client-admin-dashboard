...
export function SmartbinSourcesPieChart({ stats }: { stats: Record<string, number> }) {
  const id = "smartbin-sources-pie";
  ...
  if (chartData.length === 0) {
    return (
      <Card data-slot="card" className="@container/card @xl/main:col-span-2 gap-3">
        <CardHeader>
          <CardTitle>Drop Sources Distribution</CardTitle>
          <CardDescription>No data available for drop sources.</CardDescription>
        </CardHeader>
    ...
    <Card data-slot="card" data-chart={id} className="@container/card gap-3">
      ...
      <CardHeader className="flex flex-row items-start space-y-0 pb-0">
        <div className="grid gap-1">
          <CardTitle>Drop Sources Distribution</CardTitle>
          <CardDescription>Shows the distribution of drop sources.</CardDescription>
        </div>
