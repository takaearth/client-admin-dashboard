...
export function SmartbinSyncPieChart({ stats }: { stats: Record<string, number> }) {
  const id = "smartbin-sync-pie";
  ...
  if (chartData.length === 0) {
    return (
      <Card data-slot="card" className="@container/card gap-3">
        <CardHeader>
          <CardTitle>Drop Sync Source Distribution</CardTitle>
          <CardDescription>No data available for drop sync.</CardDescription>
        </CardHeader>
    ...
    <Card data-slot="card" data-chart={id} className="@container/card gap-3">
      ...
      <CardHeader className="flex flex-row items-start space-y-0 pb-0">
        <div className="grid gap-1">
          <CardTitle>Drop Sync Source Distribution</CardTitle>
          <CardDescription>Shows the distribution of drop sync sources.</CardDescription>
        </div>
